...
ビルドコマンド：
```bash
g++ src/main.cpp src/Renderer.cpp src/Tetromino.cpp src/Board.cpp src/GameState.cpp src/ScoreFile.cpp -o tetris.exe \
...
| 4 | ライン消去・スコア・ゲームオーバー・セーブ継続・2種クリア | ✅ 完了 |
...
├── Board.hpp/cpp      # int[20][10] グリッド・7bag・isValid・lock・clearLines
├── GameState.hpp/cpp  # ゲーム全状態・スコア・レベル・入力処理
├── ScoreFile.hpp/cpp  # ハイスコア・レベル保存（highscore.txt）
├── Renderer.hpp/cpp   # InitWindow・描画・フォント管理・オーバーレイ
├── main.cpp           # ゲームループ
```
